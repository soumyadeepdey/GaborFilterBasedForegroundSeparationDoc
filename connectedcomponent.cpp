...
nocc* IITkgp_functions::FindConnectedComponent(Mat image, Mat src_gray)
{
    ...
    for(i=0;i<row;i++)
    {
        for(j=0;j<col;j++)
        {
	    tempstructuredimage[i][j].p.x=j;
	    tempstructuredimage[i][j].p.y=i;
...
vector<vector<Point> > IITkgp_functions::Convert_nocc2pointvector(nocc *component)
{
  vector<vector<Point> > con_comp;
  
  for(int i=0;i<tncc;i++)
  {
    cc *cctemp;
    cctemp = component[i].start;
    vector<Point> p;
    while(cctemp->nxt!=NULL)
    {
      p.push_back(cctemp->im.p);
      cctemp = cctemp->nxt;
    }
    p.push_back(component[i].start->im.p);
    con_comp.push_back(p);
    p.clear();
  }
